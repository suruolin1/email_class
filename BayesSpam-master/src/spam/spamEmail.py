...
class spamEmailBayes:
    ...
    #通过计算每个文件中p(s|w)来得到对分类影响最大的15个词  计算一个词语在垃圾邮件中出现的概率
    ...
    def calAccuracy(self,testResult):
        ...
        for name ,catagory in testResult.items():
            if (int(name)<1000 and catagory==0) \
                    or(10000>int(name)>1000 and catagory==1) \
                    or(20000>int(name)>10000 and catagory==2) \
                    or(30000>int(name)>20000 and catagory==3):
                rightCount+=1
            else:
                errorCount+=1
